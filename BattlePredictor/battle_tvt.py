...
class EnemyAgent(TerranAgent):
    def __init__(self, bm):
        ...
        self.bm = bm
        self.previous_predictor_health = {}  # Para rastrear la salud de las unidades del predictor
    ...
    def step(self, obs):
        ...
        if obs.first():
            ...
            self.previous_predictor_health = {}  # Reiniciar el seguimiento de la salud

        # Calcular el daño infligido
        current_predictor_health = {}
        if hasattr(obs.observation, "raw_units"):  # Verificar si raw_units está disponible
            for unit in obs.observation.raw_units:
                if unit.alliance == features.PlayerRelative.SELF:  # Unidades del predictor
                    current_predictor_health[unit.tag] = unit.health

        # Comparar con la salud anterior para calcular el daño
        for unit_tag, health in current_predictor_health.items():
            if unit_tag in self.previous_predictor_health:
                damage = self.previous_predictor_health[unit_tag] - health
                if damage > 0:
                    self.bm.enemy_damage_inflicted += damage

        # Actualizar la salud anterior
        self.previous_predictor_health = current_predictor_health
...
def run_episode(_):
    print(f"Iniciando episodio {_}")
    flags.FLAGS([sys.argv[0]])  # Inicializar los flags correctamente
    bm = BattleManager()  # Nueva instancia de BattleManager
    agent1 = PredictorAgent(bm)  # Nueva instancia de PredictorAgent
    agent2 = EnemyAgent(bm)  # Nueva instancia de EnemyAgent
    ...
    try:
        with sc2_env.SC2Env(
            ...
            agent_interface_format=features.AgentInterfaceFormat(
                ...
                use_raw_units=True,  # Asegúrate de que esto esté habilitado
                raw_resolution=64,
            ),
            step_mul=1024,
            disable_fog=True,
            ...
            print(f"Entorno creado para el episodio {_}")
            run_loop.run_loop([agent1, agent2], env, max_episodes=1)
            print(f"Episodio {_} completado")
    except Exception as e:
        print(f"Error en el episodio {_}: {e}")
        # Reiniciar el entorno si es necesario
        os.system("taskkill /f /im SC2.exe")  # Forzar cierre de StarCraft II en Windows
...
def main(unused_argv):
    ...
    while contar_registros() < TOTAL_REGISTROS_DESEADOS:
        ...
        for i in range(num_episodios):  # Ejecutar en serie (sin multiprocessing)
            run_episode(i)
